...
class OracleLiveData:
    ...
    def login(
        ...
        elif self.brokerName == "dhan":
            if user_id != None and token != None:
                ret = self.broker.login(client_id=user_id, access_token=token)
            elif user_id is not None:
                ret=self.broker.login(client_id=user_id)

        elif self.brokerName == "paper":
            if user_id != None:
                ret = self.broker.login(user_id=user_id)

    def set_session(self, user_id=None, password=None, token=None):
        """
    ...
    def get_livedata(
        ...
        con = type(exchange) == str and type(instruments) == list and onmessage != None

        if con:
            self.broker.get_livedata(
